...
fn no_authors_fail() {
    ...
    let output = mit_hook_test_helper::run_hook(&working_dir, "git-mit", vec![]);
    let stderr = indoc!(
        "
        ...
        USAGE:
            git-mit <initials>... --config <file> --timeout <timeout>
...
fn the_file_can_be_generated_by_an_command_executed_in_the_users_shell() {
    ...
    let working_dir = mit_hook_test_helper::setup_working_dir();
    let output = mit_hook_test_helper::run_hook(&working_dir, "git-mit", vec!["-e", config, "bt"]);
