...
fn help_returned_by_long_flag() {
    ...
    let output =
        mit_hook_test_helper::run_hook(&working_dir, "mit-prepare-commit-msg", vec!["--help"]);
...
fn help_returned_by_short_flag() {
    let working_dir = mit_hook_test_helper::setup_working_dir();
    let output = mit_hook_test_helper::run_hook(&working_dir, "mit-prepare-commit-msg", vec!["-h"]);
    let expected_stdout = vec![
        format!("mit-prepare-commit-msg {}", env!("CARGO_PKG_VERSION")),
        indoc!(
            "
...
fn short_help_returned_when_a_wrong_message_commands_passed() {
    let working_dir = mit_hook_test_helper::setup_working_dir();
    let output =
        mit_hook_test_helper::run_hook(&working_dir, "mit-prepare-commit-msg", vec!["--banana"]);
